...
# TODO: grad norms are increasing -> add weight decay to optimizer  weight_decay=0.01
# TODO: Use gpt2/3 beta scheduler for optimizer (betas=(0.9, 0.98), eps=1e-9) 
...
print(f"loaded {len(en_texts)} english-hindi pairs")
...
indic_tokenizer.build_vocab(hi_texts)

en_sequences = eng_tokenizer.texts_to_sequences(en_texts)
hi_sequences = indic_tokenizer.texts_to_sequences(hi_texts)

# Split texts
en_train_texts = [en_texts[i] for i in train_indices]
en_val_texts   = [en_texts[i] for i in val_indices]
hi_train_texts = [hi_texts[i] for i in train_indices]
hi_val_texts   = [hi_texts[i] for i in val_indices]

# Split sequences
en_train = en_sequences[train_indices]
en_val   = en_sequences[val_indices]
hi_train = hi_sequences[train_indices]
hi_val   = hi_sequences[val_indices]

# split the data using your function
# en_train, en_val, hi_train, hi_val = simple_train_val_split(en_sequences, hi_sequences, test_size=0.1, random_state=42)

# convert to torch tensors
en_train_tensor = torch.tensor(en_train, dtype=torch.long)
hi_train_tensor = torch.tensor(hi_train, dtype=torch.long)
en_val_tensor = torch.tensor(en_val,     dtype=torch.long)
hi_val_tensor = torch.tensor(hi_val,     dtype=torch.long)

train_loader = DataLoaderLite(X=en_train_tensor, y=hi_train_tensor, batch_size=config.BATCH_SIZE)
val_loader = DataLoaderLite(X=en_val_tensor,     y=hi_val_tensor,   batch_size=config.BATCH_SIZE)
...
# LR Scheduler params
max_lr        = 9e-4                        # Peak learning rate after warmup, used in cosine decay schedule
...
# Training loop
checkpoint_path = os.path.join("checkpoints", "eng_hindi", wandb_experiment) 
