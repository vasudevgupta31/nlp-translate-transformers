...
transformer_model = torchtx(src_vocab_size=eng_tokenizer.vocab_size, 
                            tgt_vocab_size=hi_tokenizer.vocab_size, 
                            max_seq_len=hi_tokenizer.max_len,
                            d_model=config.D_MODEL, 
                            n_heads=config.N_HEADS, 
                            n_layers=config.N_LAYERS, 
                            d_ff=config.D_FF,
...
# Loss
loss_fn = nn.CrossEntropyLoss(ignore_index=256)          # ignore padding tokens
...
wandbrun = wandb.init(entity=wandb_entity,
                      ...
                      config={
                                "max_seq_len": config.TGT_MAX_LENGTH,
                                "steps": MAX_STEPS,
